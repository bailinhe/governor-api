...
const (
	testSystemExtensionResourcesResponse = `[
		{
	...
	testSystemExtensionResourceResponse = `{
		"id": "673ccd3a-1381-4e68-bc90-04e5f6745b9c",
		"resource": {
			"age": 10,
			"firstName": "test",
	...
	}{
		{
			...
			fields: fields{
				httpClient: &mockHTTPDoer{
					t:          t,
					resp:       []byte(testSystemExtensionResourcesResponse),
					statusCode: http.StatusOK,
				},
			},
			expected:  testResp([]byte(testSystemExtensionResourcesResponse)),
			expectErr: false,
		},
		{
			name: "non-success",
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{
				httpClient: &mockHTTPDoer{
					t:          t,
					resp:       []byte(testSystemExtensionResourceResponse),
					statusCode: http.StatusOK,
				},
			},
			expected:  testResp([]byte(testSystemExtensionResourceResponse)),
			expectErr: false,
		},
		{
